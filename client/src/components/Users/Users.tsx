...
const Users: React.FC = () => {
  ...
  const userElements: JSX.Element[] = users.map(user => {
    return (
      <tr className={`table-row`}
        ...
        <td>{user.lastName}</td>
        <td>{user.email}</td>
        <td><button className="btn btn-success" onClick={() => setUserAdmin(user)}>Approve</button> </td>
      </tr>);
  });
  ...
  const adminElements: JSX.Element[] = admins.map(admin => {
    return (
      <tr className={`table-row`}
        ...
        <td>{admin.email}</td>
        <td><button className="btn btn-danger" onClick={() => setUserNotAdmin(admin)}>Revert</button> </td>
      </tr>);
  });
  ...
  return (
    <Fragment>
      ...
      <div className="row">
        <TopCard title="Approved Users" text={admins.length.toString()} icon="user-tie" class="primary" />
        <TopCard title="Pending for Approval" text={users.length.toString()} icon="user" class="danger" />
      </div>
